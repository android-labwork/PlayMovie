...
public class SearchMovieViewModel extends ViewModel implements Callback<Movies> {
    private MutableLiveData<ArrayList<Item>> items;
    private Context context;

    public MutableLiveData<ArrayList<Item>> getItems(Context context, String API, String language, String query, int page) {
        ...
        Call<Movies> call = api.searchMovies(API, language, query, page);
    ...
    public void onResponse(Call<Movies> call, Response<Movies> response) {
        if (response.isSuccessful() && response.body() != null) {
            ...
            for (int i = 0; i < data.getItems().size(); i++) {
                Item item = new Item();
                item.setTitle(data.getItems().get(i).getTitle());
        ...
        else Log.d("SearchMovie", "Body Error");
    }

    @Override
    public void onFailure(Call<Movies> call, Throwable t) {
        Log.d("SearchMovie", "Request Error");
    }
}
